...
**Ledger state — derived, and now gated.** As of 2026-09-16, the ledger is **43 rows**
(13 Class L, 5 Class T, 25 Class O): **14 CLOSED**, 3 CLOSED — ASSUMPTION FALSE (L7's row, O21
...
| O25 | **CHAIN-CONSISTENT ORIGIN — that the hops of one dispatch chain agree about where their authority context came from.** `tla/AuthoritySelect.tla` models §6.8's authority-selection MUST, whose whole subject is a per-hop fact (*does this access serve a live caller's request?*). §6.8's context-propagation clause says the caller capability and the author are **inherited** down the chain, so a real peer's origins are RELATED along it — and a context-propagation bug is precisely a chain whose origins stop being related | **Both `AuthoritySelect` modules: a hop's `origin` is chosen FREELY at `Spawn`, not derived from the hop that spawned it.** The domain is therefore WIDER than any implementation — it admits a caller-serving hop beneath an autonomous one. That direction is safe for every property here, all of which are per-hop, and each counterexample the checkers returned is a SINGLE hop realizable at the first slot. What it excludes is the opposite claim: **nothing here says a chain's origins are consistent along it**, so a peer that propagates the right capability and the wrong liveness is outside this model | **This repo, and it is cheap but not free — the model would need a `parent` variable and an inheritance constraint, which is a DOMAIN RESTRICTION and would have to be a constant with a control rather than an `Init` conjunct (D18, and `AttestRevoke.tla` is why).** Opened 2026-09-16 WITH the model rather than after it. ⚠ Not blocked; not started | **OPEN** |
