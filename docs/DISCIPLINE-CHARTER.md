# Discipline charter — entity-core-formalization

**Tier 1, cold start.** `AGENTS-STANDARD.md` names this file: *"your repo's discipline
charter"*, read once when you need to know how work is done here. It is not loaded by
default and it is not a summary — it is the primary text for this repo's native
disciplines.

**D1–D12 are the ecosystem's** and live in `METHODOLOGY.md` §4. **Everything below is
ours, earned on our own bugs, numbered from D13 per the promotion ladder**
(`METHODOLOGY.md` §3): bit us once → an anti-pattern entry and a candidate; bit us a
second time *in a different shape* → ratified. A discipline with no enforcement point
does not count, so every one below names its file, gate or grep.

Ratified: **D13** (a gate must assert its outcome) · **D14** (fix the class, not the
instance) · **D15** (a derived number is a claim) · **D16** (two structurally different
engines) · **D17** (an unenforced obligation is a defect) · **D18** (a model's domain is
a claim) · **D20** (a matcher must not depend on the value it checks).
Candidate, not ratified: **D19** (the denominator comes from the spec).
...
`docs/outbox/ROUTING-2026-09-08-VALIDATION-SURFACE.md`.
...
controls in the same pass, not after they go green. `docs/archive/status/AUDIT-2026-08-30-LEAN-TIER.md`.
...
*Seventh instance, 2026-09-17, and it is the sixth instance's twin found in the file a public
reader opens first.* The sixth grepped the **quantifier phrasing** — `every`, `all`, `no module`
— and found four sites. `README.md` §"Project layout" carried a fifth that the sweep missed
because it does not use any of those words: ***"today every model in them is on the `core`
track"***. It was true when written, false from the day `attestation` was promoted, and it sat
through three more promotions and nine extension modules in **the most-read published file in
the repo**, with every gate green. The word `every` is in it; the sweep that found the other
four ran on 2026-09-07 and this line predates it, which means the grep was run and the hit was
read past — a quantifier inside a *layout description* does not read as a claim.
**Add to the grep list: a bare quantifier inside a paragraph whose job is orientation rather
than assertion** — a layout tree, a directory gloss, a "you are here". The repair is the one
this repo always reaches for: the value is deleted and the subject named (*a model file's track
is a property of `TRACKS.toml`, never of its directory*), with the stale sentence left standing
and dated beside it.

*Eighth instance, 2026-09-17, and it is the one where the class was fixed by an unrelated
reorganisation.* `docs/DISCIPLINE-CHARTER.md`, `docs/agents/memory/`, `docs/outbox/` and
`docs/archive/status/` all came into existence in one session, and **every cross-reference in
the tree to a moved file is an instance of this discipline's shape**. Enumerated and fixed
mechanically rather than by hand: ~70 `AGENTS.md D<n>` citations (live documents repointed to
the charter; **dated `docs/status/` snapshots deliberately NOT repointed**, because a snapshot
is history and a statement in one was true where it stood), 56 packet-path references, 15
archive-path references, four `retractcheck` witnesses, three `retractcheck` allow lists and
two `enginecount` declared sites. **The gates found all but one class**: `retractcheck` and
`enginecount` fail loudly when a declared site or witness goes missing, which is the whole
reason they were built that way. The class they could not find was the ordinary prose
cross-reference, and a path rewrite is what covers it.
⛔ **One sharp edge from the same sweep, worth more than the fix:** a `git ls-files`-driven
rewrite swept `AGENTS-STANDARD.md` — an **injected overlay that must stay byte-identical across
every repo in the ecosystem** — and changed one line in it. Nothing here would have caught it;
the sibling-repo conformance linter did, on byte-identity. **A tree-wide mechanical edit's file
set is an input set (D15), and the entries that must be excluded from it are the files this repo
does not own.** `AGENTS-STANDARD.md`, `METHODOLOGY.md` and `spec-data/**` are that set.
...
*Seventeenth shape, 2026-09-17 — **THE INPUT SET WIDENED, BY A REORGANISATION THAT WAS NOT ABOUT
THE GATE, AND THE GATE IMMEDIATELY ASSERTED SOMETHING IT HAD DECLARED IT COULD NOT.*** Every prior
shape here is an input set that narrowed, went empty, or was never the right set. This is the first
one that went the other way, and it is worth a rung because **the repair for a declared-unreachable
gap turned out not to be in the gate at all.**

`tools/retractcheck.py` excluded `docs/status/**` wholesale, and its own header named the cost in
2026-09-10: the prefix conflates a dated `CHECKPOINT`/`HANDOFF`, which really is immutable history,
with a **`ROUTING` note, which is an active outbound packet read by another repo.** R11's withdrawn
reachability bound sat in a routing note's own header table as a live claim and the scan was
*"structurally incapable of seeing it"* — written down, accepted, left. The header explained why it
was not being patched: a blanket un-exclude would fire on every packet that quotes its own
retracted phrasing in a correction section, and the distinction needed (quoted-as-history vs
asserted) *"is not worth inventing at the end of a session."*

**It was closed by moving the packets.** The doc standard puts sent packets in `docs/outbox/`,
which is not on the exclusion list, so **23 of them entered the live set in one commit** — 165
tracked files → 188 — and the scan reached, for the first time, the exact document its own comment
named as unreachable.

**Three things generalise, and the second is the one to carry.**

1. *The prediction was right and the magnitude was not.* The header said **every** routing note
   would fire. **Four of twenty-three did**, all four genuinely quoted-as-history, and **none
   asserts a retracted claim.** A cost estimate that blocks a fix is a claim like any other, and
   this one was never measured — it was reasoned about, which is what D15's corollary has said for
   six months.
2. *The distinction it said needed inventing already existed.* `allow` is per-(row, file). What it
   lacked was not expressiveness but **a reason written at each entry**, which is cheap. ⛔ **A gate
   whose remedy is "a field we do not have" deserves one look at the fields we do have** — the
   difference between the two is usually a paragraph, not a feature.
3. *A directory boundary was doing semantic work and nobody re-scoped when it moved.* The
   exclusion meant "history"; the directory meant "dated and internal"; those stopped being the
   same thing the day packets got a home of their own. This is `AGENTS-STANDARD.md`'s own warning
   about `keep_tree` — *"after declaring one, re-read every rule and re-scope every gate that keyed
   on the old boundary"* — arriving through `outbox` instead. ⛔ **When a directory moves, grep
   `tools/` for its path before you grep anything else.**

*Enforcement:* the exclusion list in `tools/retractcheck.py` now records the closure, what was
predicted, and what was measured, in place of the gap it used to declare.
⛔ **And the counterpart hazard is the mirror image, which is why this is one finding and not
two.** `entity-system-conformance` measured the same move from the other side, in a review
we were copied on: an **inbound** scanner keyed to `docs/status/ROUTING-*` prints `inbox: 0 packet(s)`
the day a counterpart moves, **byte-identically to a genuine clean scan.** Ours had that defect
when they wrote it — `entity-core-keystone` had already moved 47 packets, four addressed to us, and
two of those four had no tracker row. **So the same reorganisation widened a gate that greps our
own tree and silently narrowed one that greps someone else's**, and only the first is visible from
inside. The second is fixed in `docs/status/INBOUND.md` by globbing both homes, naming a
counterpart with neither as `NO PACKET HOME`, writing `UNREACHABLE` rather than omitting a row, and
carrying the watermark — which is immune because it is keyed to a **counterpart**, not to a path.
...
`docs/outbox/ROUTING-2026-09-09-KEYSTONE-SCOPE-SUBSET-TYPING.md`); named here rather than gated,
...
holds. Full correction and the reframed ask: `docs/outbox/ROUTING-2026-09-08-VALIDATION-SURFACE.md`.
...
findings — N1 and N2, `docs/outbox/ROUTING-2026-09-09-IDENTITY-HANDLE-CACHE-KEY.md` — on a
...
- **The obligation with no enforcing operation.** Nine of the twenty-one routed spec findings
  share one shape (`docs/archive/status/SEVERITY-2026-09-08-FINDINGS-TRIAGE.md` §4): a normative
