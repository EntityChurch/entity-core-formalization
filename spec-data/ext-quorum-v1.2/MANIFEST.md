# spec-data ext-quorum-v1.2 — Snapshot Manifest

**Spec:** `EXTENSION-QUORUM.md` **1.2**
**Snapshot type:** verbatim copy of the authoritative normative spec file(s) — **byte-for-byte, no paraphrase.**
**Purpose:** the modeling ground truth for the `quorum` proof track (`TRACKS.toml`). Models are
...
> **Vendored is not pinned.** This snapshot exists; whether the `quorum` models transcribe it is
...
|---|---|---|---|
| `EXTENSION-QUORUM.md` | 1.2 | 46 712 | `f892e083a0209c8a01d3f494e62dccb7344382cf14c1f8375888e7ee7d2e49c0` |

Verify: `make specfreeze` (or `sha256sum spec-data/ext-quorum-v1.2/*.md`).
...
| Vendored | 2026-09-07, by this repo, via `tools/vendor-spec.py vendor --track quorum` |
...
| Dependency floor | `ENTITY-CORE-PROTOCOL.md` (v7.40+); `EXTENSION-ATTESTATION.md` (v1.2+) — both met by the vendored snapshots |
